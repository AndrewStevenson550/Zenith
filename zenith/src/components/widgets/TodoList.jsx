...
function TodoList() {
  ...
  const deleteTask = (id) => setTodo(todo.filter(task => task.id !== id));

  return (
    <div className='bg-[#1a222e] text-white p-6 rounded-2xl flex flex-col gap-3 shadow-2xl border border-gray-800/50 w-full max-w-sm h-[180px]'>
      ...
      <input 
        ...
        className="w-full bg-[#252d3a] border border-gray-700/50 rounded-xl py-1.5 px-3 text-sm text-white outline-none focus:border-blue-500/50"
      />
      ...
      <div className="flex flex-col gap-2 overflow-y-auto pr-1">
        {todo.map((task) => (
          <div key={task.id} className="flex items-center justify-between bg-[#252d3a]/30 p-2 rounded-xl border border-gray-800/40 group">
            ...
            <button onClick={() => deleteTask(task.id)} className="text-gray-500 hover:text-red-400 text-[9px] font-bold uppercase opacity-0 group-hover:opacity-100 transition-opacity">Delete</button>
          </div>
        ))}
      </div>
    </div>
